...
export interface IVidhideService {
  ...
  searchTV(title: string, season?: number, episode?: number): Promise<VidhideSource[]>;
...
export class VidhideService implements IVidhideService {
   ...
   */
  getEmbedUrl(sourceUrl: string): string {
    // If the URL is already an embed URL, return as-is
    if (sourceUrl.includes('/embed/')) return sourceUrl;
    // Convert a file URL to an embed URL
    const fileId = sourceUrl.split('/').pop()?.split('.').shift();
    if (fileId) {
      return `${this.baseUrl}/embed/${fileId}`;
    }
    return sourceUrl;
  }
  ...
  private parseSources(data: unknown): VidhideSource[] {
    ...
    const results = Array.isArray(obj.results) ? obj.results : Array.isArray(obj) ? obj : [];
